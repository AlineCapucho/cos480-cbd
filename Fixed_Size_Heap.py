...
class Fixed_Size_Heap:
        ...
        for i in range(1, self.number_of_records+1):
            ...
            try:
                ...
                if i % self.blocking_factor == 0:
                    ...
                    block = ''
                if remaining_records != 0 and i == self.number_of_records:
                    additional_padding = " " * (self.blocking_factor - remaining_records)
    ...
    def _search(self, field_id, value, file):
        ...
        success = False
        number_of_blocks = self.number_of_records // self.blocking_factor
        ...
        for i in range(0, number_of_blocks):
            ...
            for j in range(0, self.blocking_factor):
                offset = header_length + self.block_size * i + i + self.record_size * j
                # Check if end of file
                file.seek(offset, 0)
                record = file.read(self.record_size).strip()
                if record == '':
                    break
    ...
    def _insert(self, record, file):
        ...
        block_id = self.number_of_records // self.blocking_factor
        record_id = self.number_of_records % self.blocking_factor
        ...
        if self.deleted_records:
            ...
            # Check if the block is still available
            if self.number_of_records % self.blocking_factor == 0:
                # If it's a new block, we should write a new block
                padding_size = self.block_size - self.record_size
                padding = " " * padding_size
                block = record + padding + '\n'
            else:
                # Otherwise write to an existing block
                head = block[:self.record_size * record_id]
                tail = block[self.record_size * (record_id + 1):]
                block = head + record + tail + '\n'
